...
start: prepare
	...
	docker run -d --name $(CONTAINER_NAME) --network=host -u $(id -u):$(id -g) -v /var/run/docker.sock:/var/run/docker.sock -v "$(WORKSPACE):/home/coder/project:z" $(LOCAL_IMAGE_NAME) --host 0.0.0.0 --cert --security-opt seccomp=unconfined
...
start_remote: prepare
	...
	docker run -d --name $(CONTAINER_NAME) --network=host -u $(id -u):$(id -g) -v /var/run/docker.sock:/var/run/docker.sock -v "$(WORKSPACE):/home/coder/project:z" -e PASSWORD=1234 $(REMOTE_IMAGE_NAME) --host 0.0.0.0 --cert --security-opt seccomp=unconfined 
